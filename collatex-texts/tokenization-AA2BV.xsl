...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="app" mode="tokens">
        ...
        <xsl:variable name="A2tokens" select="tokenize(rdg[@wit eq '#A2'],'\s+')"/>
        ...
        <xsl:for-each select="1 to $count">
            <xsl:choose>
                <xsl:when test="$Atokens[current()] eq $Btokens[current()] and $Atokens[current()] eq $Vtokens[current()] and $Atokens[current()] eq $A2tokens[current()]">
                ...
                <xsl:when test="$Btokens[current()] eq $Vtokens[current()] and $Btokens[current()] ne $Atokens[current()] and $Atokens[current()] eq $A2tokens[current()]">
                    <app>
                        <rdg wit="#A #A2"><xsl:value-of select="replace($Atokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#B #V"><xsl:value-of select="replace($Btokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                </xsl:when>
                <xsl:when test="$Btokens[current()] ne $Vtokens[current()] and $Btokens[current()] ne $Atokens[current()] and $Atokens[current()] eq $A2tokens[current()]">
                    <app>
                        <rdg wit="#A #A2"><xsl:value-of select="replace($Atokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                ...
                <xsl:when test="$Btokens[current()] eq $Vtokens[current()] and $Btokens[current()] ne $Atokens[current()] and $Atokens[current()] ne $A2tokens[current()]">
                    <app>
                        <rdg wit="#A"><xsl:value-of select="replace($Atokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#A2"><xsl:value-of select="replace($A2tokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#B #V"><xsl:value-of select="replace($Btokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                </xsl:when>
                <xsl:when test="$Btokens[current()] eq $Atokens[current()] and $Btokens[current()] ne $Vtokens[current()] and $Atokens[current()] eq $A2tokens[current()]">
                    <app>
                        <rdg wit="#A #A2 #B"><xsl:value-of select="replace($Atokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#V"><xsl:value-of select="replace($Vtokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                </xsl:when>
                <xsl:when test="$Atokens[current()] eq $Vtokens[current()] and $Btokens[current()] ne $Vtokens[current()] and $Atokens[current()] eq $A2tokens[current()]">
                    <app>
                        <rdg wit="#A #A2 #V"><xsl:value-of select="replace($Atokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#B"><xsl:value-of select="replace($Btokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                ...
                <xsl:otherwise>
                    <app>
                        ...
                        <rdg wit="#A2">
                            <xsl:choose>
                                <xsl:when test="$A2tokens[current()]">
                                    <xsl:value-of select="replace($A2tokens[current()], '_', ' ')"
                                        disable-output-escaping="yes"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <gap/>
