...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Stylesheet to tokenize a parallel segmentated transcription.
        ...
        Input:
            ...
            <l n="6">
                <app>
                    <rdg wit="#T">tanto uiuer per nulla ren</rdg>
    ...
    <xsl:template match="app" mode="tokens">
        <xsl:variable name="Ttokens" select="tokenize(rdg[@wit eq '#T'],'\s+')"/>
        <xsl:variable name="Btokens" select="tokenize(rdg[@wit eq '#B'],'\s+')"/>
        <xsl:variable name="Vtokens" select="tokenize(rdg[@wit eq '#V'],'\s+')"/>
        <xsl:variable name="count" select="max((count($Ttokens),count($Btokens)))"/>
        <xsl:for-each select="1 to $count">
            <xsl:choose>
                <xsl:when test="$Ttokens[current()] eq $Btokens[current()] and $Ttokens[current()] eq $Vtokens[current()]">
                    ...
                    <xsl:value-of
                        select="insert-before(replace($Ttokens[current()], '_', ' '), 2, '')"
                        disable-output-escaping="yes"/>
                </xsl:when>
                <xsl:when test="$Btokens[current()] eq $Vtokens[current()] and $Btokens[current()] ne $Ttokens[current()]">
                    <app>
                        <rdg wit="#T"><xsl:value-of select="replace($Ttokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#B #V"><xsl:value-of select="replace($Btokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                </xsl:when>
                <xsl:when test="$Btokens[current()] eq $Ttokens[current()] and $Btokens[current()] ne $Vtokens[current()]">
                    <app>
                        <rdg wit="#T #B"><xsl:value-of select="replace($Ttokens[current()], '\*', ' ')"
                            disable-output-escaping="yes"/></rdg>
                        <rdg wit="#V"><xsl:value-of select="replace($Vtokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                    </app>
                </xsl:when>
                <xsl:when test="$Ttokens[current()] eq $Vtokens[current()] and $Btokens[current()] ne $Vtokens[current()]">
                    <app>
                        <rdg wit="#T #V"><xsl:value-of select="replace($Ttokens[current()], '_', ' ')"
                            disable-output-escaping="yes"/></rdg>
                ...
                <xsl:otherwise>
                    <app>
                        <rdg wit="#T">
                            <xsl:choose>
                                <xsl:when test="$Ttokens[current()]">
                                    <xsl:value-of select="replace($Ttokens[current()], '_', ' ')"
                                        disable-output-escaping="yes"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <gap/>
