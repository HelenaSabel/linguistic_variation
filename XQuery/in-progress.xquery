...
<svg
    ...
    <g
        transform="translate(80,400)">
        {   
            for $section at $pos in distinct-values($songs//tei:head//tei:rdg[@wit eq '#A']/tei:locus/replace(@from, 'v|r', ''))
            let $ling := $songs[.//tei:head//tei:rdg[@wit eq '#A']/tei:locus/replace(@from, 'v|r', '') = $section]/descendant::tei:rdg[tokenize(replace(@ana, '#', ''), '\s+') = $ling-features]
