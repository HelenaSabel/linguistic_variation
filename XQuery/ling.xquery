    ...
    <div
        ...
        <section
            class="phenom">
            {
                ...
                return
                    (<h2><span
                            class="pt">{$ling-features[@xml:id = $result]//tei:string[@xml:lang eq 'pt']/text()}</span><span
                            class="en">{$ling-features[@xml:id = $result]//tei:string[@xml:lang eq 'en']/text()}</span></h2>,
                    <h3
                        ...
                        <tbody>
                            {
                                ...
                                return
                                    <tr>
                                        <td
                                            class="intro">{
                                                (if ($fen//tei:choice) then
                                                    $fen//tei:reg/string()
                                                else
                                                    (if (string-length($fen) = 0) then
                                                        (<span
                                                            class="normal pt">omissão</span>,
                                                        <span
                                                            class="normal en">omision</span>)
                                                    else
                                                        transform:transform($fen, 'xmldb:exist:///db/VTLGP/xslt/ex.xsl', ())
                                                    )), ' (',
                                                <span
                                                    class="normal">vs</span>, '. ',
                                                (if ($fen/../*[not(@wit = $fen/@wit)]/tei:choice) then
                                                    $fen/../*[not(@wit = $fen/@wit)]//tei:reg/string()
                                                else
                                                    if (string-length($fen/../*[not(@wit = $fen/@wit)]) = 0) then
                                                        (<span
                                                            class="normal pt">omissão</span>,
                                                        <span
                                                            class="normal en">omision</span>)
                                                    else
                                                        $fen/../*[not(@wit = $fen/@wit)]/transform:transform(., 'xmldb:exist:///db/VTLGP/xslt/ex.xsl', ())), ')'
                                            }</td>
                ...
                <ul>
                    {
                        for $result in $values
                        return
                            <li>{$ling-features[./@xml:id = $result]//tei:string[@xml:lang eq 'en']}
                                <ul>
                                    <li><a
